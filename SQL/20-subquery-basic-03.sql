-- 20-subquery-basic-03.sql

USE lecture;

-- 각 카테고리 평균 매출 중에서 50만원 이상
SELECT 
	category,
...
-- 영업사원별 성과 등급 분류 [영업사원, 총매출액, 주문건수, 평균주문액, 매출등급, 주문등금]
...
ORDER BY 총매출액 DESC;
-- 여러 번 같은 내용을 쓰지 않고 가상 테이블을 만들어 행 이름을 적어주는 이유: 따로 적게되면 다시 전부 계산을 들어가기 때문에