...
SELECT
	name,
	tags,
	tags[1] AS first_tag, -- 첫 번째 태그를 가져오는 것이 가능
	'PostgreSQL' = ANY(tags) AS pg_dev -- 값이 존재할 때에 true, 없을 때에 false
FROM datatype_demo;
...
SELECT
	name,
	metadata, -- 정보에 대한 정보
	metadata->> 'department' AS 부서, -- 두쌍 화살표: text 추출
	metadata->'skills' AS 능력 -- 한 화살표: jsonb 추출
FROM datatype_demo;
...
SELECT
	...
	location[0] AS 위도,
	location[1] AS 경도,
	POINT(37.505027,127.005011) <-> location AS 고터까지거리
FROM datatype_demo;